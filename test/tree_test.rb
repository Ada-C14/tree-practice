...
describe Tree do
  ...
  let (:another_tree) { Tree.new }

  let (:another_tree_with_nodes) do
    tree.add(5, 'Peter')
    tree.add(3, 'Paul')
    tree.add(1, 'Mary')
    tree.add(10, 'Karla')
  ...
  describe 'delete' do
    it 'will delete a leaf node' do
      ...
      expect(tree_with_nodes.height).must_equal 2
    end
    
    it 'will delete a node with a right child and replace with correct node' do
      another_tree_with_nodes.delete(15)
      replacement_node = another_tree_with_nodes.find_return_node(23)

      expect(replacement_node.right.key).must_equal 25
      expect(replacement_node.left.key).must_equal 13
    end

    it 'will delete a node with a left child and replace with correct node' do
      another_tree_with_nodes.delete(3)
      replacement_node = another_tree_with_nodes.find_return_node(1)

      expect(replacement_node.right).must_be_nil
      expect(replacement_node.left).must_be_nil
    end
  end
end
